...
The library includes **complete HTML documentation** (29 files) covering every class, feature, and test case, making it easy to learn and use. **Every source and test file includes EIS (Eiffel Information System) links** that provide F1 context-sensitive help directly in EiffelStudio—press F1 on any class to instantly view its documentation.
...
### 7. Entity Serialization Pattern

Implement JSON serialization for your domain entities:
...
- `has_all_keys (<<"field1", "field2">>)` - Check multiple required fields at once
- `missing_keys (array)` - Get list of missing required fields
- `integer_32_item ("key")` - Direct INTEGER_32 extraction (no `.to_integer_32` needed)
- `optional_string ("key")` - Returns detachable for optional fields
- `optional_integer ("key", default)` - Returns default if key missing
- `optional_boolean ("key", default)` - Returns default if key missing
...
├── src/                       # Source code (25 files with EIS)
...
├── testing/                   # Test suites (13 files with EIS)
...
- **Author:** Larry Rix
- **Repository:** https://github.com/simple-eiffel/simple_json
