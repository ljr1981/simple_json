...
feature {NONE} -- Test Runners
	...
	run_simple_json_tests
		do
			...
			run_test (agent (create {TEST_UNICODE_TEXT}).test_astral_round_trip_through_object, "test_astral_round_trip_through_object")
			run_test (agent (create {TEST_UNICODE_TEXT}).test_surrogate_pair_escapes_combine, "test_surrogate_pair_escapes_combine")
			run_test (agent (create {TEST_UNICODE_TEXT}).test_escaper_vectors, "test_escaper_vectors")
			run_test (agent (create {TEST_UNICODE_TEXT}).test_array_and_facade_strings, "test_array_and_facade_strings")
